              ...
              <form @submit.prevent="handleSubmit">
                <div class="card-body">
                    ...
                    <div class="col-md-6 pl-md-1">
                        ...
                        <input type="email" v-model="employee.email" id="email" class="form-control" placeholder="name@email.com">
                        </div>
                    </div>
                    </div>
                    <div class="row">
    ...
    <script>
        new Vue({
            ...
            methods: {
                ...
                async handleSubmit() {
                    const { name, email, position_held, salary, work_type, status, status_time, address, city, country, postal_code } = this.employee;
                    if (name && email && position_held && salary && work_type && status && status_time && address && city && country && postal_code) {
                        const data = { name, email, position_held, salary, work_type, status, status_time, address, city, country, postal_code };
                        this.isLoading = true;
                        try {
